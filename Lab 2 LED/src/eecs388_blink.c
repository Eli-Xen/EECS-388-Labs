...
{
   int gpioR = RED_LED;
   int gpioB = BLUE_LED; //To see the actual pin numbers, look for GREEN_LED (or the other colors) in the eecs388_lib.h file
   int gpioG = GREEN_LED; //GREEN_LED is a macro. It is essentially a placeholder that is replaced by the compiler during compilation into the correct pin number
     
   gpio_mode(gpioR, OUTPUT); //this function sets the pin referred to by the variable gpio to act as an OUTPUT pin, i.e. send signals out of it
   ...
   while(1)
   {
      gpio_write(gpioR, ON); // the gpio_write() function sets a pin (referred to by gpio in this case) to the logic high or ON state.
      ...
      gpio_write(gpioR, OFF); // same as before, but this time we are setting the pin to logi low or OFF state. Same as turning the pin off
      delay(300); //this makes the light turn off for a little before turning it on again 

      gpio_write(gpioB, ON); 
      delay(1000); 
      gpio_write(gpioB,OFF); 
      delay(300); 
      ...
      //turning them all on at the same time makes the LED white 
      gpio_write(gpioR, ON); 
      gpio_write(gpioB, ON); 
      gpio_write(gpioG, ON); 
      delay(1000); 
